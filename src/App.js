import React, { Component } from 'react';
import { render } from 'react-dom';
import { Stage, Layer, Rect, Text, Group } from 'react-konva';
import Konva from 'konva';
import MathJax from 'react-mathjax';

class Parameter extends React.Component {
    constructor(props) {
        super(props);
    }

    render() {
        return (
            <div className="parameter">
                <MathJax.Node className="name" inline formula={this.props.symbol} />
                <input value={this.props.value} onChange={(e) => this.props.onParameterChange(this.props.symbol, e.target.value)}/>
            </div>
        );
    }
}

class Constant extends React.Component {
    constructor(props) {
        super(props);
    }

    render() {
        return (
            <div className="constant">
                <MathJax.Node className="name" inline formula={this.props.symbol} />
                <input value={this.props.value} readOnly />
            </div>
        );
    }
}

class Pin extends React.Component {
    constructor(props) {
        super(props);
    }

    render() {
        if (this.props.anchor === "left") {
            return (
                <div className="pin pin-left">
                    <div className="connector"></div>
                    <MathJax.Node className="name" inline formula={this.props.name} />
                </div>
            );
        } else {
            return (
                <div className="pin pin-right">
                    <MathJax.Node className="name" inline formula={this.props.name} />
                    <div className="connector"></div>
                </div>
            );
        }

    }
}

class Helper {
    static getVariables(str) {
        return str.match(/[A-Za-z_]+/g);
    }
}

class ArrayHelper {
    static flatten(array) {
        return array.reduce((acc, e) => acc.concat(e), []);
    }

    static unique(array) {
        return Array.from(new Set(array));
    }
}

class Node extends React.Component {
    constructor(props) {
        super(props);

        const formula = this.props.formula;
        const left = formula.split("=")[0];
        const right = formula.split("=")[1];

        this.state = {
            left: Helper.getVariables(left),
            right: Helper.getVariables(right),
            mode: "view",
            inputFormula: formula,
        };
    }

    formatFormula(formula) {
        return formula.replace(/\*/g, " \\cdot ").replace(/([A-Za-z_\d]+)\/([A-Za-z_\d]+)/g, "\\dfrac{$1}{$2}");
    }

    variables(stateVariables, anchor) {
        const variables = [];
        for (let variable of stateVariables) {
            variables.push(<Pin key={variable} name={variable} anchor={anchor} />);
        }
        return variables;
    }

    edit() {
        this.setState({
            ...this.state,
            mode: "edit",
        });
    }

    save() {
        this.setState({
            ...this.state,
            mode: "view",
        });
        this.props.onFormulaChange(this.props.id, this.state.inputFormula);
    }

    handleChange(e) {
        this.setState({
            ...this.state,
            inputFormula: e.target.value,
        });
    }

    renderButton() {
        if (this.state.mode === "view") {
            return <button onClick={() => this.edit()}>edit</button>;
        } else {
            return <button onClick={() => this.save()}>save</button>;
        }
    }

    render() {
        const style = {
            left: this.props.x,
            top: this.props.y,
        };
        const formulaStyle = {
            visibility: this.state.mode === "view" ? "visible": "hidden",
        };
        const inputStyle = {
            display: this.state.mode === "view" ? "none": "flex",
        };

        return (
            <div className="node" style={style}>
                <div>
                    {this.variables(this.state.left, "left")}
                </div>
                <div className="math">
                    <div className="formula-container">
                        <MathJax.Node style={formulaStyle} formula={this.formatFormula(this.props.formula)} />
                        <div style={inputStyle} className="formula">
                            <input value={this.state.inputFormula} onChange={(e) => this.handleChange(e)} />
                        </div>
                    </div>
                    {this.renderButton()}
                </div>
                <div>
                    {this.variables(this.state.right, "right")}
                </div>
            </div>
        );
    }
}

class App extends Component {
    constructor(props) {
        super(props);
        this.state = {
            x: 20,
            nodes: [
                {
                    id: 1,
                    formula: "1/2*m*v^2=E",
                    x: 600,
                    y: 300,
                },
                {
                    id: 2,
                    formula: "m*g*h=E",
                    x: 600,
                    y: 500,
                },
            ],
            constants: [
                {
                    symbol: "g",
                    value: "9.81",
                },
            ],
            parameters: [
                {
                    symbol: "m",
                    value: "2",
                },
                {
                    symbol: "v",
                    value: "3",
                },
            ],
        };

        // const parameters = {"appName":"web3d",appletOnLoad:() => this.onLoadCAS(),"id":"applet1","width":435,"height":185,"showToolBar":false,"borderColor":null,"showMenuBar":false,"showAlgebraInput":false,"showResetIcon":true,"enableLabelDrags":false,"enableShiftDragZoom":true,"enableRightClick":false,"capturingThreshold":null,"showToolBarHelp":false,"errorDialogsActive":true,"useBrowserForJS":false,"ggbBase64":""};
        // this.applet = new window.GGBApplet(parameters, '5.0');
        // this.applet.inject('applet_container1');
    }

    onLoadCAS() {
        this.loadCAS();
    }

    triggerCAS() {
        return window.applet1 && window.applet1.evalCommandCAS && window.applet1.evalCommandCAS("2*x") === "2x";
    }

    loadCAS() {
        if (this.triggerCAS()) {
            console.log("CAS loaded.");
            this.onLoadedCAS();
            return;
        }
        setTimeout(() => this.loadCAS(), 200);
    }

    casCommand(command) {
        console.log(command);
        return window.applet1.evalCommandCAS(command);
    }

    casSolve(equations, variables) {
        const command = "Solve({" + equations.join(",") + "}, {" + variables.join(",") + "})";
        return this.casCommand(command);
    }

    onLoadedCAS() {
        const equationsNodes = this.state.nodes.map(node => node.formula);
        const equationsConstants = this.state.constants.map(constant => constant.symbol + "=" + constant.value);
        const equationsParameters = this.state.parameters.map(parameter => parameter.symbol + "=" + parameter.value);
        const equations = [...equationsNodes, ...equationsConstants, ...equationsParameters];
        const variables = ArrayHelper.unique(ArrayHelper.flatten(equations.map(equation => Helper.getVariables(equation))));
        const result = this.casSolve(equations, variables);
        console.log("result", result);
    }

    onMouseMove(e) {
    }

    parameters() {
        const parameters = [];
        for (let constant of this.state.parameters) {
            parameters.push(<Parameter key={constant.symbol} symbol={constant.symbol} value={constant.value} onParameterChange={(symbol, value) => this.handleParameterChange(symbol, value)} />);
        }
        return parameters;
    }

    constants() {
        const constants = [];
        for (let constant of this.state.constants) {
            constants.push(<Constant key={constant.symbol} symbol={constant.symbol} value={constant.value} />);
        }
        return constants;
    }

    handleParameterChange(symbol, value) {
        console.log("ffdddddf", symbol, value);
    }

    handleFormulaChange(id, value) {
        console.log("fff", id, value);
    }

    nodes() {
        const nodes = [];
        for (let node of this.state.nodes) {
            nodes.push(<Node key={node.id} id={node.id} formula={node.formula} x={node.x} y={node.y} onFormulaChange={(id, value) => this.handleFormulaChange(id, value)} />);
        }
        return nodes;
    }

    render() {
        return (
            <MathJax.Provider>
                <div onMouseMove={(e) => this.onMouseMove(e)}>
                    {this.nodes()}
                    <div className="constants-container">
                        {this.constants()}
                    </div>
                    <div className="parameters-container">
                        {this.parameters()}
                    </div>
                </div>
            </MathJax.Provider>
        );
    }
}

export default App;
